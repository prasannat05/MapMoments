...
app.factory('FeedService', function () {
  ...
  return {
    addPing: function (data) {
      ...
      // Add default reactions when adding a new ping
      data.reactions = { like: 0, love: 0, fun: 0 };
...
app.run(function ($rootScope, AuthService, $location) {
  ...
  $rootScope.isDark = localStorage.getItem('mm_darkMode') === 'true';

  $rootScope.toggleDarkMode = function () {
    $rootScope.isDark = !$rootScope.isDark;
    document.body.classList.toggle('dark-mode', $rootScope.isDark);
    localStorage.setItem('mm_darkMode', $rootScope.isDark);
  };
